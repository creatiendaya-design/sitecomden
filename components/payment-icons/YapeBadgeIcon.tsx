"use client";

import { useId } from "react";

interface YapeBadgeIconProps {
  className?: string;
  width?: number;
  height?: number;
}

/**
 * Yape en formato tarjeta (38×24, con marco propio), para la fila de marcas
 * del checkout. Distinto de `YapeIcon`, que es el logo cuadrado suelto.
 *
 * Los ids de gradiente y clip se generan por instancia: el mismo icono puede
 * aparecer dos veces en la página y los ids duplicados hacen que la segunda
 * copia referencie el `<defs>` de la primera.
 */
export default function YapeBadgeIcon({
  className = "",
  width = 38,
  height = 24,
}: YapeBadgeIconProps) {
  const uid = useId().replace(/:/g, "");
  const gradientId = `yape-badge-gradient-${uid}`;
  const clipId = `yape-badge-clip-${uid}`;

  return (
    <svg
      viewBox="0 0 38 24"
      width={width}
      height={height}
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      role="img"
      aria-label="Yape"
    >
      <title>Yape</title>
      <defs>
        <linearGradient
          id={gradientId}
          x1="12.61"
          x2="24.72"
          y1=".93"
          y2="21.9"
          gradientTransform="matrix(1 0 0 -1 0 24)"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0" stopColor="#571a7c" />
          <stop offset=".32" stopColor="#5b1c7f" />
          <stop offset=".57" stopColor="#662287" />
          <stop offset=".8" stopColor="#762b98" />
          <stop offset=".91" stopColor="#8031a2" />
        </linearGradient>
        <clipPath id={clipId}>
          <path fill="none" d="M8.63 1.63h20.74v20.74H8.63z" />
        </clipPath>
      </defs>
      <path
        d="M35 0H3C1.3 0 0 1.3 0 3v18c0 1.7 1.4 3 3 3h32c1.7 0 3-1.3 3-3V3c0-1.7-1.4-3-3-3"
        opacity=".07"
        style={{ isolation: "isolate" }}
      />
      <path
        fill="#fff"
        d="M35 1c1.1 0 2 .9 2 2v18c0 1.1-.9 2-2 2H3c-1.1 0-2-.9-2-2V3c0-1.1.9-2 2-2z"
      />
      <g clipPath={`url(#${clipId})`}>
        <path
          fill={`url(#${gradientId})`}
          d="M25.58 22.37H12.42a3.8 3.8 0 0 1-3.79-3.79V5.42a3.8 3.8 0 0 1 3.79-3.79h13.16a3.8 3.8 0 0 1 3.79 3.79v13.16a3.8 3.8 0 0 1-3.79 3.79"
        />
        <path
          fill="#fff"
          d="M23.39 14.11c-1.01.02-2.16.13-3.21.26.12-.43.24-.85.24-.85s.12.26.46.23c.49-.04 1.08-.51 1.36-1.38.3-.92.28-1.97-.34-2.05-.5-.07-.74.37-.78.46 0 0 .11-.4-.21-.33-.31.07-.36.2-.44.45-.01.04-.27 1-.54 2.01v.02c-.07.23-.2.3-.29.31-.31.02.35-2.01.4-2.16.07-.2.11-.36-.12-.44-.15-.06-.36-.04-.42.13-.03.07-.04.12-.04.12s-.17-.26-.55-.22c-.38.03-.86.5-1.18 1.32s-.46 1.99.3 2.1c.38.06.71-.16.88-.52 0 0-.03.24.22.36.1.05.35.03.58-.1-.06.23-.12.45-.17.64-1.64.23-2.92.48-2.92.48l1.1-3.7c.17-.56-.15-.44-.38-.37-.25.07-.37.28-.5.67-.55 1.68-.98 2.2-1.29 2.09-.29-.1.62-1.88.4-2.12-.19-.2-.58-.19-.65-.07s.04.23-.01.4-.41 1.09-.45 1.3-.35 1.35.48 1.3c.49-.03.83-.4.86-.5-.15.44-.37 1.16-.37 1.16s-1.01.25-1.81 1.09c-.64.67-1.09 2.1-.14 2.33 1.48.36 2.18-1.9 2.53-2.75 1.2-.27 2.11-.44 2.96-.58-.01.05-.02.08-.02.1-.08.35-.06.49.22.46.3-.04.33-.19.43-.56 0-.03.01-.06.02-.09 1-.14 1.98-.22 3.34-.33.42-.04.42-.11.46-.36.05-.28-.28-.29-.38-.29zm-2.58-1.95c.2-.66.39-.93.56-1 .42-.18.5.34.17 1.14-.37.92-.81.9-.97.72 0 0 .02-.16.24-.85zm-1.86.4c-.18.43-.35.73-.58.74-.33 0-.32-.49-.09-1.12.2-.52.56-.86.75-.89.19-.04.42.04-.09 1.27zm-4.36 5.02c-.79.7-.59-.26-.3-.67.55-.79 1.24-.99 1.24-.99-.35 1.02-.78 1.52-.94 1.66"
        />
        <path
          fill="#fff"
          d="M24.44 11.57c.3-.34.43-1.08-.07-1.35-.6-.33-1.39.17-1.71 1.24-.29.95-.4 2.13.58 2.08.48-.02.9-.37 1.05-.63.07-.13.1-.32.09-.41-.04-.25-.19-.28-.44.04-.06.07-.57.52-.81.25-.18-.19-.03-.57-.02-.57.32-.02.88-.15 1.33-.67zm-.72-.84c.27-.25.53.05.22.48-.32.44-.68.43-.69.44.04-.17.18-.65.47-.93z"
        />
        <path
          fill="#13c9b2"
          d="M19.3 5.52c-1.06.05-1.92.9-1.99 1.96-.04.72.27 1.36.79 1.77.05.04.08.11.08.17v.46s.02.07.05.08h.01s.07.02.09 0l.4-.29c.05-.04.12-.05.18-.04q.24.06.48.06c1.16 0 2.09-.94 2.09-2.09s-.99-2.14-2.19-2.09zm.24 2.71c-.05.08-.13.14-.23.19s-.22.07-.36.07c-.16 0-.3-.03-.41-.09a.4.4 0 0 1-.18-.18.4.4 0 0 1-.07-.22c0-.04.01-.08.04-.1.03-.03.06-.04.11-.04q.06 0 .09.03c.02.02.05.06.06.1.02.05.04.1.07.13.02.04.06.06.1.09q.06.03.18.03c.1 0 .18-.02.24-.07s.09-.1.09-.17c0-.06-.02-.1-.05-.13a.4.4 0 0 0-.13-.08q-.075-.03-.21-.06a1.5 1.5 0 0 1-.3-.1.6.6 0 0 1-.19-.15c-.05-.06-.07-.14-.07-.24s.03-.17.07-.24a.4.4 0 0 1 .22-.16c.09-.04.21-.06.33-.06.1 0 .19.01.26.04s.14.06.19.1.09.09.11.13c.02.05.03.09.03.13s-.01.08-.04.11-.06.05-.11.05c-.04 0-.07 0-.09-.03s-.04-.05-.07-.09a.6.6 0 0 0-.11-.15c-.04-.04-.11-.05-.2-.05q-.135 0-.21.06c-.05.04-.08.08-.08.14 0 .03 0 .06.03.09a.3.3 0 0 0 .17.1c.03 0 .09.02.16.04.09.02.18.05.25.07s.14.06.19.1.09.09.12.14c.03.06.04.13.04.21 0 .1-.03.19-.08.27zm1.01-1.39s-.02.07-.05.14l-.55 1.38s-.03.08-.05.11-.03.05-.05.06-.05.02-.08.01c-.08-.01-.12-.05-.11-.13 0-.02.02-.07.05-.14l.55-1.38c.03-.07.05-.12.07-.15s.05-.04.1-.03c.04 0 .07.02.09.04s.03.05.02.09z"
        />
      </g>
    </svg>
  );
}
